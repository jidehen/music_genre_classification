...
class Atten_Head(tf.keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):		
		...
		self.W_K = self.add_weight("K",shape=[input_size, output_size])
		self.W_V = self.add_weight("V",shape=[input_size, output_size])
		self.W_Q = self.add_weight("Q",shape=[input_size, output_size])
	...
	def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
		...
		attn_mat = Attention_Matrix(K, Q, self.use_mask) 
		return tf.matmul(attn_mat, V)
